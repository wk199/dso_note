...
struct FrameHessian
{
	...
	// variable info.
	SE3 worldToCam_evalPT;   //-- 当前帧的真实pose信息
	Vec10 state_zero;    //-- 这个就是优化开始的点，为了使用FEJ保存的信息咯？每一帧的开始和结束给其置为0
	Vec10 state_scaled;
	Vec10 state;	      // [0-5: worldToCam-leftEps. 6-7: a,b]   //-- 优化时，增量部分。真正的pose部分是worldToCam_evalPT
	Vec10 step;  //-- 本次优化产生的步长(增量)
	Vec10 step_backup; //-- 上一次优化产生的步长(增量)
	Vec10 state_backup;  //-- 新一次的迭代优化前，保存上一时刻的优化增量
    ...
    //-- 这里的先验，对于第一帧，包括所有变量，对于非第一帧，只包括光学参数
...
struct CalibHessian
{
	...
	//-- 这里保存的是相机内参在优化时的增量相关的所有信息。和FrameHessian中的一样
	VecC value_zero;
	VecC value_scaled;
	VecCf value_scaledf;
	VecCf value_scaledi;
