...
//	meanStepC += Hcalib.step.norm();
	...
	if(setting_solverMode & SOLVER_MOMENTUM)
	{
		...
		for(FrameHessian* fh : frameHessians)
		{
			...
			fh->setState(fh->state_backup + step);   //-- 这是在更新当前每帧的增量信息：state、state_scaled
			sumA += step[6]*step[6];
			sumB += step[7]*step[7];
...
float FullSystem::optimize(int mnumOptIts)
{
	...
	for(int iteration=0;iteration<mnumOptIts;iteration++)
	{
		...
		bool canbreak = doStepFromBackup(stepsize,stepsize,stepsize,stepsize,stepsize);  //-- 优化结束后，更新所有变量的状态信息和增量信息
