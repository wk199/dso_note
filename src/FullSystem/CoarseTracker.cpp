		...
		FrameHessian* frame)
{
	...
	for(FrameHessian* fh : frameHessians)
	{
		...
		for(PointHessian* ph : fh->pointHessians)
		{
			...
			fwdWarpedIDDistFinal[u+w1*v]=0; //-- 把投影成功的点记录下来
			bfsList1[numItems] = Eigen::Vector2i(u,v);
			numItems++;
		}
	}

	//-- 以这些投影的点为圆心，标记周围的点到这些点的距离。因此叫距离地图
...
void CoarseDistanceMap::growDistBFS(int bfsNum)
{
	...
	for(int k=1;k<40;k++)
	{
		...
		//-- 当K=1时，遍历这些投影点，标记其周围的八个点，其距离值设为k，并存储这些做了标记的点
        //-- 当k=2时，遍历k=1时存储的点，标记其周围的四个点，其距离值设为k，并存储这些做了标记的点
        //-- 当k=3时，遍历k=2时存储的点。。。。。
        //-- 直到k=40
