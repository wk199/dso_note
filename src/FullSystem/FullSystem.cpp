...
void FullSystem::activatePointsMT()
{
	...
	//-- 根据ef中现有的点的数量，来调整currentMinActDist，控制再次增加的的点的数量
	//-- currentMinActDist表示当前未成熟点与已有成熟点在当前帧中的距离阈值。注意，都投影到当前帧中来
	...
	//-- 构建距离地图：
    //-- 将历史关键帧中的active点投影到当前帧中来，一这些投影的点构建同心园，这样就知道了每一个点距离已有active点的距离
	// make dist map.
...
//				immature_invalid_deleted++;
			...
			bool canActivate = (ph->lastTraceStatus == IPS_GOOD    //-- 此处是只要不是IPS_OUTLIER就有可能被激活
...
//				immature_notReady_skipped++;
			...
			if((u > 0 && v > 0 && u < wG[1] && v < hG[1]))
			{
				...
				//--  将历史帧中的某一帧的未成熟点投影到当前帧，判断其离成熟点的距离
				//-- 距离超过一定值才会被选取，为了保证点分布的稀疏性吧
				float dist = coarseDistanceMap->fwdWarpedIDDistFinal[u+wG[1]*v] + (ptp[0]-floorf((float)(ptp[0])));

				if(dist>=currentMinActDist* ph->my_type)  //-- my_type变量在ImmaturePoint对象构建时传入
				{
					coarseDistanceMap->addIntoDistFinal(u,v);  //-- 当前点即将被优化，也即将成熟，就将其也存入距离地图中
		...
		for(unsigned int i=0;i<host->pointHessians.size();i++)
		{
			...
			else if(ph->isOOB(fhsToKeepPoints, fhsToMargPoints) || host->flaggedForMarginalization)
			{
				...
				if(ph->isInlierNew())
				{
					flag_in++;
					int ngoodRes=0;
					for(PointFrameResidual* r : ph->residuals)   //-- 把要边缘化的帧的点先全部置成isLinearized，
					{
						r->resetOOB();
						r->linearize(&Hcalib);
						...
						{
							r->efResidual->fixLinearizationF(ef);  //-- 将isLinearized设置为true
							ngoodRes++;
						}
					}


					//-- 把上面残差置成isLinearized的点，一部分边缘化掉，一部分丢掉。
...
void FullSystem::addActiveFrame( ImageAndExposure* image, int id )
{
    ...
    fh->makeImages(image->image, &Hcalib);   //-- Hcalib是setGammaFunction()中设置的标定的gamma响应，同时包含各种标定参数
                                             //-- 构建图像金字塔
...
{
	// ========================== initial FrameHessian  information  ======================
	{
		boost::unique_lock<boost::mutex> crlock(shellPoseMutex);
		assert(fh->shell->trackingRef != 0);
		fh->shell->camToWorld = fh->shell->trackingRef->camToWorld * fh->shell->camToTrackingRef;
	...
	// 3、=========================== add New Frame to Hessian Struct. =========================
...

	{
		...
		coarseTracker_forNewKF->setCoarseTrackingRef(frameHessians);   //-- 将当前帧更新为coarseTracker_forNewKF中的参考帧，下一帧用
                                                                       //-- pc_color、pc_idepth、pc_u、pc_v
...
void FullSystem::setPrecalcValues()
{
	...
	ef->setDeltaF(&Hcalib);   //-- 更新EFFrame的delta_prior、delta
}
