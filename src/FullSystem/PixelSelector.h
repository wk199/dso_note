		...
		for(int x=1;x<w-pot;x+=pot)
		{
			...
			for(int dx=0;dx<pot;dx++)
				for(int dy=0;dy<pot;dy++)
				{
					...
					float sqgd = g.tail<2>().squaredNorm();  //-- 两个方向的梯度的平方和
					float TH = THFac*minUseGrad_pixsel * (0.75f);  //-- 当前层的点数 × minUseGrad_pixsel * (0.75f)
			...
			//-- bestXXID、bestYYID、bestXYID、bestYXID可能为同一个，有可能为四个不同的idx
			//-- 对于每一个栅格，numGood最大增加3
			if(bestXXID>=0)
			{
				if(!map0[bestXXID])
					numGood++;
...
inline int makePixelStatus(Eigen::Vector3f* grads, bool* map, int w, int h, float desiredDensity, int recsLeft=5, float THFac = 1)
{
	...
	else if(sparsityFactor==5) numGoodPoints = gridMaxSelection<5>(grads, map, w, h, THFac);  //-- map会先被重置，然后再存储当前层的
